...
In {art:rif.1} viene descritto che cos'è un \textit{sensore}:
\begin{quote}
...
In \cite{art:rif.11} vengono fornite due classificazioni per i trasduttori, la prima in base al segnale in uscita:
\begin{quote}	
	\begin{enumerate}
	...
	\end{enumerate}
\end{quote}
Mentre la seconda classificazione è in base alla necessità di energia esterna per il loro funzionamento:
\begin{quote}
...
Il testo continua classificando i segnali in tre tipi:
\begin{quote}
	\begin{enumerate}
...
Per leggere un segnale bisogna utilizzare un trasduttore. In \cite{art:rif.2} vengono descritte le caratteristiche importanti che deve possedere un trasduttore per compiere una lettura del segnale: 
\begin{quote}
...
\end{quote}
Il testo continua descrivendo due importanti operazioni da effettuare sul segnale:
\begin{quote}	
